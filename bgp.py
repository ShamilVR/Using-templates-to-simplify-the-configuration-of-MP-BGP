...
''')
    ...
    while True:
        ...
        return_to_previous = input(
            "Do you want to add another network or proceed with BGP configuration? (yes/BGP): ").lower()
...
def configure_mp_bgp():
    ...
    if router_role in ['pe', "provider's edge router", "provider's edge"]:
        vrf_configs = []
...
def main():
    ...
    configuration = "\n".join(
        interface_configs + [ospf_process_number] + ospf_configs + [bgp_asn_config] + bgp_neighbor_configs + vrf_configs)
